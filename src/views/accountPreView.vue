			...
			<div class="com-width">
				...
				<div v-if="openStatus != 6">
					<div class="progress-ctn">
						<div :class="[index <= state ? 'active' : '']" class="progress" v-for="(val, index) in openStatusList" :key="index">
					...
					<div v-if="(openStatus==0 || openStatus==3 || openStatus==4 || openStatus==5)" class="btn-openAccount active"
					 @click="goPage(openStatusList[oIndex].path)">{{ openStatusList[oIndex].btnText }}</div>
				...
				<div v-if="openStatus == 6" class="main">
						...
						<div class="item">
							<div class="label">
								<span>
									...
									<tips top="0" right="-20px">{{$t('accountPreview.text5')}}</tips>
								</span>
							</div>
							<div class="amount">0</div>
						</div>
					...
					<div class="wrap2 clear">
						<div class="l-w fl">
							...
							<div @click="$router.push('/assetsConfig')" class="fr detail pointer">{{$t('accountPreview.text7')}}</div>
							<span id="type">{{ type }}</span>
							<div ref="echartss" id="echartss"></div>
							<div class="legend">
								<div @mouseover="highlight(index)" @mouseout="downplay(index)" v-for="(item,index) in investList" :key="index"
								 class="legend-item">
						...
						<div class="r-w fr">
							...
							<div class="cont">
								...
								<div class="rw-amount rw-amount">{{ money }}</div>
								<div class="line"></div>
					...
					<div class="fundRecords">
						<div class="top-wrap">
							...
							<div @click="$router.push('/userCenter/fundRecords')" class="records pointer">{{$t('accountPreview.text21')}}>></div>
						</div>
						<table>
							<thead>
