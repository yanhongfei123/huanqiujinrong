  ...
  <div class="trans-wrap">
    ...
    <div class="com-width">
      ...
      <div class="trans-content clear">
        <div class="l-cont fl">
          <div class="label">交易密码说明</div>
          <div class="info">1.为了您的资金安全，在配置资产前请先设置交易密码。</div>
          <div class="info">2.交易密码用于配置资产、调整配置等场景下使用，可在个人中心修改。</div>
        </div>
        <div class="r-cont fl">
          <div class="page-trans">
            <div class="content">
              <el-form class="form" :model="regForm" :rules="regRules" ref="regForm">
                ...
                <div class="btn btn-register" @click="confirm('regForm')">设置交易密码</div>
              </el-form>
            </div>
          </div>
        </div>
...
export default {
  ...
  data() {
    ...
    const validatePass = (rule, value, callback) => {
      let pwdReg = /^\d{6}$/;
      if (value == "") {
        callback(new Error("请输入6位交易密码"));
      } else if (!pwdReg.test(value)) {
        callback(new Error("请输入6位数字的交易密码"));
      } else {
        callback();
      }
    };
